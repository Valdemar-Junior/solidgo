...
// Rota de status para verificar se o servidor está online via Browser
app.get("/", (req, res) => {
    res.send(`
        <html>
            <head><title>DANFE Server</title></head>
            <body style="font-family: sans-serif; padding: 20px;">
                <h1>Servidor DANFE Online 🚀</h1>
                <p>O endpoint de geração está ativo em: <code>POST /danfe/html-base64</code></p>
                <p>Envie um JSON com o campo <code>xml</code> para gerar o DANFE.</p>
            </body>
        </html>
    `);
});

app.post("/danfe/html-base64", async (req, res) => {
    try {
...
app.listen(PORT, () => {
    ...
    console.log("Endpoint: POST http://localhost:" + PORT + "/danfe/html-base64");
});
// ================================================
