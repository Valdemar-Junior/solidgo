DROP FUNCTION IF EXISTS public.get_product_commitment_report(text, date, date, text[], integer, integer);
...
REVOKE ALL ON FUNCTION public.get_product_commitment_report(text, date, date, text[], text[], integer, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_product_commitment_report(text, date, date, text[], text[], integer, integer) TO authenticated;

COMMENT ON FUNCTION public.get_product_commitment_report(text, date, date, text[], text[], integer, integer)
...
CREATE INDEX IF NOT EXISTS idx_orders_product_commitment_lote_status_sale
ON public.orders (import_source, status, data_venda)
WHERE import_source = 'lote'
  AND COALESCE(return_flag, false) = false;

CREATE OR REPLACE FUNCTION public.get_product_commitment_storage_locations()
RETURNS text[]
LANGUAGE sql
STABLE
SECURITY INVOKER
...
REVOKE ALL ON FUNCTION public.get_product_commitment_storage_locations() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_product_commitment_storage_locations() TO authenticated;

COMMENT ON FUNCTION public.get_product_commitment_storage_locations()
IS 'Retorna somente os locais distintos de pedidos em lote para preencher o filtro com baixo egress.';
