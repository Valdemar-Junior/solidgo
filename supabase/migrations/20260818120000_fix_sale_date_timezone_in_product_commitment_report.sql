-- Corrige o filtro de periodo de venda do relatorio de produtos comprometidos.
--
-- data_venda e timestamptz e a sessao do banco roda em UTC, entao `data_venda::date`
-- devolvia o dia seguinte para vendas a partir das 21h de Brasilia. Agora o corte do
-- dia acontece no fuso de Brasilia, igual ao que as telas exibem.
--
-- Somente o WHERE do periodo de venda mudou; assinatura, retorno e permissoes
-- seguem identicos, por isso CREATE OR REPLACE basta.
...
BEGIN
  ...
  WITH expanded AS (
    ...
    WHERE o.status IN ('pending', 'imported', 'assigned', 'delivered')
      ...
      AND (p_sale_start IS NULL OR (o.data_venda AT TIME ZONE 'America/Sao_Paulo')::date >= p_sale_start)
      AND (p_sale_end IS NULL OR (o.data_venda AT TIME ZONE 'America/Sao_Paulo')::date <= p_sale_end)
  ), scoped AS (
    SELECT *
      FROM expanded e
     WHERE (
