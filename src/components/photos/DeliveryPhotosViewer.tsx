/**
 * DeliveryPhotosViewer - Botão + Modal para visualizar fotos de entrega
 * 
 ...
 * 
 * Uso:
 * <DeliveryPhotosViewer routeOrderId="uuid-do-route-order" />
 */

import { useState, useEffect } from 'react';
import { Camera, X, Loader2, ImageOff, Truck } from 'lucide-react';
import { supabase } from '../../supabase/client';
import { PhotoGallery, PhotoItem } from './index';

export interface DeliveryPhotosViewerProps {
    /** ID do route_order para buscar as fotos */
    routeOrderId: string;
...
export default function DeliveryPhotosViewer({
    routeOrderId,
    size = 'sm',
    showEmpty = false,
}: DeliveryPhotosViewerProps) {
    ...
    useEffect(() => {
        const fetchCount = async () => {
            try {
                ...
                console.error('[DeliveryPhotosViewer] Erro ao buscar contagem:', err);
                setPhotoCount(0);
            } finally {
                setIsLoading(false);
            }
    ...
    const loadPhotos = async () => {
        try {
            ...
            const photosWithUrls: PhotoItem[] = await Promise.all(
                data.map(async (record: PhotoRecord) => {
                    ...
                    return {
                        ...
                        alt: `${record.photo_type}: ${record.file_name}`,
            ...
            console.error('[DeliveryPhotosViewer] Erro ao carregar fotos:', err);
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
                        ...
                        <div className="bg-green-600 text-white px-6 py-4 flex items-center justify-between flex-shrink-0">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-lg font-bold">Fotos da Entrega</h2>
