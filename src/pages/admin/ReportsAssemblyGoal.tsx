...
import { ArrowDown, ArrowLeft, ArrowRight, CalendarRange, Eye, FileSpreadsheet, Hammer, Loader2, RotateCcw, Route, UserCircle2 } from 'lucide-react';
...
import {
  AssemblyGoalReportGenerator,
  type AssemblyGoalPersonRow,
  type AssemblyGoalReportData,
  type AssemblyGoalRouteBreakdown,
  type AssemblyGoalWeekSection,
} from '../../utils/pdf/assemblyGoalReportGenerator';
...
type CompletedRouteRow = {
  ...
  assembly_products?: AssemblyProductWithDetails[] | null;
};
...
const MONTHLY_PERFORMANCE_DELIVERY_TARGET = 80;
...
const evaluateWeeklyTarget = (received: number, delivered: number) => {
  ...
  return {
    ...
    analysis: `Recebeu ${received} montagens no periodo e realizou ${delivered}. A meta semanal de ${WEEKLY_TARGET} montagens foi ${quantityTargetMet ? 'atingida' : 'nao atingida'}.`,
  };
};

const routeLabel = (route: AssemblyGoalRouteBreakdown) => {
...
export default function ReportsAssemblyGoal() {
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        ...
        setPersonOptions(
          ((data || []) as Array<{ id: string; name?: string | null; active?: boolean | null }>)
  ...
  const generatePdf = async () => {
    try {
      ...
      const pdfBytes = await AssemblyGoalReportGenerator.generate(reportData);
      ...
      <main className="w-full space-y-6 p-4 sm:p-6 lg:p-8">
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Relatorio de meta de montagem</h1>
              <p className="mt-1 text-sm text-gray-500">
                Avalie montadores com base nas rotas de montagem finalizadas, respeitando a contagem consolidada de produtos e kits.
              </p>
            </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
            <FilterCard
              ...
              title="Periodo"
              description="Selecione o periodo mensal. O consolidado usa metas fixas e os blocos semanais exigem 25 montagens."
            >
            ...
            <FilterCard
              ...
              title="Regras"
              description="Cada produto avulso vale uma montagem. Componentes de kit sao consolidados pela regra de receita do sistema."
            >
              <div className="rounded-lg border border-violet-100 bg-white px-3 py-2 text-xs leading-5 text-gray-600">
                Semanal: 25. Mensal: 100. Desempenho: 80 montagens com no minimo 90%.
              </div>
            </FilterCard>
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <SummaryTile
            icon={<Hammer className="h-5 w-5 text-blue-700" />}
            title="Montadores avaliados"
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-gray-500">Montado</th>
...
function ResultBadge({
  ...
  result: AssemblyGoalPersonRow['finalResult'];
}) {
  const className =
...
async function fetchPreviewData(filters: FiltersState): Promise<PreviewData> {
  ...
  routes.forEach((route) => {
    ...
    const stats = calculateAssemblyStats(route.assembly_products || []);
    const received = stats.totalItems;
    const delivered = stats.completedItems;
    const returned = stats.returnedItems;

    const routeBreakdown: AssemblyGoalRouteBreakdown = {
      routeId: route.id,
    ...
    const normalizedId = String(route.assembler_id || '');
    const normalizedName = normalizeText(route.assembler?.name);
    if (!normalizedId || !normalizedName) return;
    if (filters.personIds.length > 0 && !filters.personIds.includes(normalizedId)) return;

    const completionDay = new Date(completedDate.getFullYear(), completedDate.getMonth(), completedDate.getDate());
    const weekIndex = weeks.findIndex((week) => completionDay >= week.start && completionDay <= week.end);
    const key = `installer:${normalizedId}`;

    const monthlyEntry = monthlyMap.get(key) || createAggregationBucket(normalizedName, 'Montador');
    ...
    if (weekIndex >= 0) {
      const weeklyEntry = weeklyMaps[weekIndex].get(key) || createAggregationBucket(normalizedName, 'Montador');
  ...
  const buildRows = (
    ...
    evaluate: (received: number, delivered: number) => Omit<AssemblyGoalPersonRow, 'personId' | 'personName' | 'personType' | 'received' | 'delivered' | 'returned' | 'routes'>
  ) =>
    Array.from(map.entries())
      .map(([personId, entry]) => {
        ...
        return {
          ...
          ...evaluation,
        } satisfies AssemblyGoalPersonRow;
      })
