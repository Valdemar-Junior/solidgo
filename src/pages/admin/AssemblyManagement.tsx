...
function AssemblyManagementContent() {
  ...
  const loadData = async (silent: boolean = true) => {
    try {
      ...
      console.log('[AssemblyManagement] routes:', Array.isArray(routesData) ? routesData.length : 0, 'ms:', Math.round(tRoutes - t0));
      ...
      console.log('[AssemblyManagement] pending:', Array.isArray(productsPending) ? productsPending.length : 0, 'ms:', Math.round(tPending - tRoutes));
      ...
      const tInRoutes = performance.now();
      console.log('[AssemblyManagement] inRoutes:', Array.isArray(productsInRoutes) ? productsInRoutes.length : 0, 'ms:', Math.round(tInRoutes - tPending));
      ...
      const { data: montadoresData } = await supabase
        ...
        .select('*')
        .eq('role', 'montador');
      const tMont = performance.now();
      console.log('[AssemblyManagement] montadores:', Array.isArray(montadoresData) ? montadoresData.length : 0, 'ms:', Math.round(tMont - tInRoutes));
      ...
      console.log('[AssemblyManagement] vehicles:', Array.isArray(vehiclesData) ? vehiclesData.length : 0, 'ms:', Math.round(tVeh - tMont));
      ...
      try {
        ...
        if (orderIds.length > 0) {
          ...
          if (isMountedRef.current) setDeliveryInfo(map);
        } else {
          if (isMountedRef.current) setDeliveryInfo({});
        }
      } catch {}
      const tRO = performance.now();
      console.log('[AssemblyManagement] route_orders delivered ms:', Math.round(tRO - tVeh));
      const tDone = performance.now();
      console.log('[AssemblyManagement] loadData total ms:', Math.round(tDone - t0));
  ...
  const createAssemblyRoute = async () => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from('assembly_products')
        .update({ 
          assembly_route_id: routeData.id, 
          installer_id: selectedMontador || null 
        })
        .in('id', productIds);
              ...
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
                  ...
                  <div className="p-6 space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          <div>
                              <label className="block text-sm font-medium text-gray-700 mb-2">Montador</label>
                              <select 
                  ...
                  <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
                      ...
                      >
                        {saving ? 'Salvando...' : 'Confirmar Romaneio'}
                      </button>
                  </div>
              </div>
          </div>
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            const products = assemblyProducts.filter(ap => ap.assembly_route_id === route.id);
                            const orders = products.map(p => p.order).filter(Boolean) as any[];
