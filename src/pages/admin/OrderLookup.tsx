  ...
  const reprintReturnDanfe = async (returnEvent: ReturnEventInfo) => {
    ...
    try {
      ...
      if (!base64.startsWith('JVBER')) {
        ...
        // Gera a DANFE de devolução pelo endpoint do proprio projeto (/api/danfe), a partir do
        ...
        if (!generated.startsWith('JVBER')) {
          toast.error('Não foi possível gerar a nota de devolução (verifique o XML / servico de DANFE).');
          return;
        }
        await supabase
          .from('orders')
