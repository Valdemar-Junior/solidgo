...
export type DeliveryGoalRouteBreakdown = {
  routeId: string;
  routeName: string;
...
export type DeliveryGoalWeekSection = {
  label: string;
...
export class DeliveryGoalReportGenerator {
  static async generate(data: DeliveryGoalReportData): Promise<Uint8Array> {
    ...
    const resultColor = (result: DeliveryGoalPersonRow['finalResult']) => {
    ...
    const drawHeader = () => {
      drawText('RELATORIO DE META DE ENTREGA', margin, y, 18, true, rgb(0.08, 0.16, 0.28));
    ...
    const drawSummary = () => {
      ...
      const cards: SummaryCard[] = [
        { label: 'Pessoas avaliadas', value: data.monthlyRows.length, color: rgb(0.18, 0.18, 0.2) },
